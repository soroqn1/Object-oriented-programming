...
from .models import Assignment, Member, Priority, ProjectState, TaskItem, TaskStatus


class MemberService:
    def __init__(
        self, members: IRepository[Member], assignments: IRepository[Assignment]
    ):
        self.members = members
        self.assignments = assignments

    def add(
        self, first_name: str, last_name: str, email: str | None, capacity: float | None
    ) -> Member:
        ...
        *,
        ...
        if first_name is not None:
            if not first_name.strip():
                raise DomainValidationException("first_name empty")
            m.first_name = first_name.strip()
        if last_name is not None:
            if not last_name.strip():
                raise DomainValidationException("last_name empty")
    ...
    def find_members(self, keyword: str) -> list[Member]:
        ...
        return self.members.find(
            lambda m: kw in m.first_name.lower()
            or kw in m.last_name.lower()
            or (m.email and kw in m.email.lower())
        )
...
class TaskService:
    def __init__(
        self,
        tasks: IRepository[TaskItem],
        members: IRepository[Member],
        assignments: IRepository[Assignment],
    ):
        self.tasks = tasks
        self.members = members
        self.assignments = assignments
    ...
    def assign(self, task_id: UUID, member_id: UUID, hours: float) -> Assignment:
        ...
        load = sum(
            a.allocated_hours
            for a in self.assignments.find(lambda a: a.member_id == member_id)
        )
...
class ReportingService:
    ...
    def get_project_state(self) -> ProjectState:
        ...
        active = sum(
            1 for t in all_tasks if t.status in (TaskStatus.Todo, TaskStatus.InProgress)
        )
        overdue = len(
            [
                t
        ...
        return ProjectState(
            total=total, done=done, active=active, overdue=overdue, by_member_load=loads
        )
...
class TaskService(TaskService):  # extend
    def update_task(
        ...
        *,
        ...
        if title is not None:
            if not title.strip():
                raise DomainValidationException("Title empty")
            t.title = title.strip()
        if description is not None:
            t.description = description
        if priority is not None:
            if priority not in Priority.choices:
                raise DomainValidationException("Invalid priority")
            t.priority = priority
        if due_at is not None:
            if due_at.date() < datetime.utcnow().date():
                raise DomainValidationException("Due date must be in the future.")
            t.due_at = due_at
        if status is not None:
            if status not in TaskStatus.choices:
                raise DomainValidationException("Invalid status")
            t.status = status
            if status == TaskStatus.Done:
                t.progress = 100
        self.tasks.update(t)
        ...
        *,
        ...
        if only_overdue is True:
            ...
            tasks = [
                t
                for t in tasks
                if t.due_at and t.due_at < now and t.status != TaskStatus.Done
            ]
        if sort_by:
    ...
    def search_by_member(self, member_id: UUID) -> list[TaskItem]:
        task_ids = {
            a.task_id for a in self.assignments.find(lambda x: x.member_id == member_id)
        }
        return [t for t in self.tasks.get_all() if t.id in task_ids]

    def search_members_of_task(self, task_id: UUID) -> list[Member]:
        member_ids = {
            a.member_id for a in self.assignments.find(lambda x: x.task_id == task_id)
        }
        return [m for m in self.members.get_all() if m.id in member_ids]

    def search_tasks_by_status_and_term(
        self, *, status_done: bool | None = None, term: str | None = None
    ) -> list[TaskItem]:
        now = datetime.utcnow()
