...
def member_add(ctx: typer.Context,
    ...
    c = ctx.obj
    m = c.member_service.add(first_name, last_name, email, capacity)  # type: ignore
...
def member_update(ctx: typer.Context, member_id: str,
    ...
    c = ctx.obj
    m = c.member_service.update_member(UUID(member_id), first_name=first_name, last_name=last_name, email=email, capacity=capacity)  # type: ignore
...
def member_delete(ctx: typer.Context, member_id: str):
    c = ctx.obj
    c.member_service.delete_member(UUID(member_id))  # type: ignore
...
def task_add(ctx: typer.Context,
    ...
    t = c.task_service.add(title, prio, due_dt, desc)  # type: ignore
...
def task_view(ctx: typer.Context, task_id: str):
    c = ctx.obj
    t = c.task_service.tasks.get(UUID(task_id))  # type: ignore
...
def task_update(ctx: typer.Context, task_id: str,
    ...
    c = ctx.obj
    due_dt = datetime.strptime(due, "%Y-%m-%d") if due else None
    t = c.task_service.update_task(UUID(task_id), title=title, description=desc, priority=prio, due_at=due_dt, status=status)  # type: ignore
...
def task_delete(ctx: typer.Context, task_id: str):
    c = ctx.obj
    c.task_service.delete_task(UUID(task_id))  # type: ignore
...
def task_list2(ctx: typer.Context,
    ...
    c = ctx.obj
    tasks = c.task_service.list_tasks(status=status, only_overdue=(True if overdue else None), sort_by=sort_by)  # type: ignore
    for t in tasks:
...
def search_by_member(ctx: typer.Context, member_id: str):
    c = ctx.obj
    tasks = c.task_service.search_by_member(UUID(member_id))  # type: ignore
...
def search_members_of_task(ctx: typer.Context, task_id: str):
    c = ctx.obj
    members = c.task_service.search_members_of_task(UUID(task_id))  # type: ignore
...
def task_assign(ctx: typer.Context, task_id: str, member_id: str, hours: float):
    c = ctx.obj
    a = c.task_service.assign(UUID(task_id), UUID(member_id), hours)  # type: ignore
...
def task_set_status(ctx: typer.Context, task_id: str, status: str):
    c = ctx.obj
    c.task_service.set_status(UUID(task_id), status)  # type: ignore
    rprint({"ok": True})
...
def task_set_progress(ctx: typer.Context, task_id: str, progress: int):
    c = ctx.obj
    c.task_service.set_progress(UUID(task_id), progress)  # type: ignore
    rprint({"ok": True})
...
def report_state(ctx: typer.Context):
    c = ctx.obj
    state = c.reporting_service.get_project_state()  # type: ignore
    rprint({
        "total": state.total,
        "done": state.done,
        "active": state.active,
